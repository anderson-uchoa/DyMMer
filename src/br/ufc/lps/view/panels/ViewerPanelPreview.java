...
public class ViewerPanelPreview extends JPanel {
	...
	private void inicializarTreePerfuse(){
 	...
 	public ViewerPanelPreview(final ContextModel model, File file, Main main) {
...
//		
			...
			public void mouseReleased(MouseEvent event) {
				...
				if (event.getButton() == MouseEvent.BUTTON1) {
					 ...
					 if (event.getSource() == listaPossibilidadesAdaptacoes) {
						...
						ViewerPanelPreview.this.preenchendoArvore(adaptacoesDisponiveis.get(selectedConstraintIndex), true);
	...
	private void setTreeVisualization(final String contextName) {
		...
		if(model.getContexts().isEmpty()){
			JOptionPane.showMessageDialog(ViewerPanelPreview.this, "It's not supported, because it does not have any context. Please, first edit it and add one context.");
			tree.setModel(null);
			return;
		}	
